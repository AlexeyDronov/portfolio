            ...
            <div className="flex flex-col gap-8">
                {blogs.map((blog) => (
                    <Link key={blog.slug} href={`/blog/${blog.slug}`} className="group">
                        <article className="flex flex-col md:flex-row md:items-baseline justify-between gap-2 md:gap-8 p-4 -mx-4 rounded-[4px] transition-colors hover:bg-slate-900/50 hover:shadow-[0_0_20px_rgba(168,85,247,0.05)]">
                            ...
                            <div className="md:text-right shrink-0">
                                ...
                                <span className="font-mono text-xs text-primary mt-1 inline-block border border-primary/20 px-2 py-0.5 rounded-[2px] bg-primary/5">
                                    {blog.topic}
                                </span>
                            </div>
                        </article>
